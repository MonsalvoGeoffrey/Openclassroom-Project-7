...
import full_star from '../assets/fullStar.png';
import grey_star from '../assets/greyStar.png';
...
function Host() {
    ...
    const host = data.find(host => host.id === hostId);
    const [picture, setPicture] = useState(0);
    if (host === undefined) {
        return <Navigate to="/404" replace />;
    }
    const [hostFirstName, hostLastName] = host.host.name.split(' ');

    function setAndWrapPicture(count) { setPicture((count + host.pictures.length) % host.pictures.length); }
            ...
            <main className="main-content">
                <section className='host-picture-container'>
                    ...
                    {host.pictures.length > 1 ?
                        <React.Fragment>
                            <p className='host-picture-count'>{picture + 1}/{host.pictures.length}</p>
                            <img className='host-picture-left' alt='left-arrow' role='button' src={arrow_back} onClick={() => { setAndWrapPicture(picture - 1) }} />
                ...
                <section className='information'>
                    ...
                    <div className='information-right-side'>
                        <div className='information-host'>
                            <p className='information-host-name'>{hostFirstName}<br />{hostLastName}</p>
                            <img className='profile-picture' src={host.host.picture} alt={host.host.name} />
                        </div>
                        <div className='information-star-container'>
                            {Array.from({ length: 5 }).map((_, index) => {
                                return <img className='star' alt='' key={index} src={index < parseInt(host.rating) ? full_star : grey_star} />
                            })}
                        </div>
                    </div>
                </section>
